...
public:
  ...
  raceDemogData(double FN, double A, double B, double L, double HPI, double MR, double W, double WNH, int size) {
    initializeCounts(FN, A, B, L, HPI, MR, W, WNH, size); 
  ...
  void initializeCounts(double FN_percent, double A_percent, double B_percent, double L_percent, double HPI_percent, double MR_percent, double W_percent, double WNH_percent, int totalPop) {
    FirstNationCount = int(FN_percent*(totalPop*1.0) );
    AsianCount = int(A_percent*(totalPop*1.0) );
    BlackCount = int(B_percent*(totalPop*1.0) );
    LatinxCount = int(L_percent*(totalPop*1.0) );
    HIPacificIsleCount = int(HPI_percent*(totalPop*1.0) );
    MultiRaceCount = int(MR_percent*(totalPop*1.0) );
    WhiteCount = int(W_percent*(totalPop*1.0) );
    WhiteNHCount = int(WNH_percent*(totalPop*1.0) );
    //otherCount = int(other_percent*(totalPop*1.0) );
    communityCount = totalPop; 
  }
